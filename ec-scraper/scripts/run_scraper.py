        ...
        for pending_url in pending:
            ...
            try:
                ...
                try:
                    ...
                    rprint(f"  [yellow]⚠ Warning: Failed to add embedding: {e}[/yellow]")
                    # Continue anyway - opportunity is saved even if embedding fails
...
async def _stats():
    ...
    try:
        ...
        pg_count = len(client.table('opportunities').select('id').execute().data)
        active_count = len(client.table('opportunities').select('id').eq('is_active', True).execute().data)
    except Exception as e:
...
async def _list_all(limit: int):
    ...
    try:
        await sync.connect()
        client = sync._get_client()
        response = client.table('opportunities').select(
            'id,title,organization,category,ec_type,extraction_confidence,deadline'
        ...
        for row in rows:
            ...
            table.add_row(
                row['title'][:40],
                row.get('organization', 'Unknown')[:30],
                row.get('category', 'Other'),
                row.get('ec_type', 'Other'),
                conf,
            )
        
        console.print(table)
    except Exception as e:
        rprint(f"[red]✗ Error fetching opportunities: {e}[/red]")
    finally:
        await sync.close()
